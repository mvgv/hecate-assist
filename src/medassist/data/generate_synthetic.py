...
TEMPLATES = {
    "laudo.md": (
        ...
        "---\n\n"
        "## 1. Quando usar\n"
        "Usar este modelo para registrar o resultado interpretado de qualquer exame de imagem, "
        "endoscópico ou anatomopatológico solicitado pela equipe assistencial. O laudo é emitido "
        "pelo médico executante do exame e anexado ao prontuário eletrônico em até 24 horas do "
        "procedimento; resultados críticos exigem comunicação verbal imediata ao médico "
        "solicitante, além do registro escrito.\n\n"
        ...
        "Laudo — Radiografia de tórax PA e perfil. Paciente: [PACIENTE]. Data: [DATA]. "
        "Descrição: opacidade heterogênea em lobo inferior direito, com broncograma aéreo, sem "
        "derrame pleural significativo; seios costofrênicos livres; área cardíaca dentro dos "
        "limites da normalidade. Conclusão: achados compatíveis com processo infeccioso "
        "pulmonar em lobo inferior direito; correlacionar com quadro clínico e laboratorial. "
        "Médico executante: [MEDICO] — [CRM].\n\n"
        "## 4. Regras de preenchimento\n"
        "A seção Descrição registra apenas achados objetivos, sem hipótese diagnóstica; a "
        "interpretação fica na Conclusão e deve usar linguagem de probabilidade (compatível com, "
        "sugestivo de, não se pode excluir), nunca afirmação categórica de diagnóstico. "
        "Identificar o paciente somente pelo identificador institucional, jamais por nome "
        "completo ou documento. Todo laudo exige assinatura e CRM do médico executante para ter "
        "validade; o assistente virtual pode sugerir a redação, mas o conteúdo final é de "
        "responsabilidade e validação do médico responsável.\n\n"
        "---\n"
    ...
    "receita.md": (
        ...
        "---\n\n"
        "## 1. Quando usar\n"
        "Usar este modelo na prescrição de alta hospitalar ou ambulatorial. A receita é sempre "
        "emitida e assinada por médico habilitado; medicamentos sujeitos a controle especial "
        "exigem receituário próprio e via adicional retida pela farmácia. O assistente virtual "
        "não emite receitas — apenas apresenta o modelo e apoia a redação, cabendo ao médico "
        "responsável definir fármaco, dose e via.\n\n"
        "## 2. Estrutura do documento\n"
        "    # Receita Médica\n\n"
        "    **Paciente:** {{paciente_id}}\n"
        "    **Data:** {{data}}\n\n"
        ...
        "Cada item de {{itens}} segue o padrão: nome do fármaco, concentração, forma "
        "farmacêutica, via de administração, intervalo e duração do tratamento.\n\n"
        "## 3. Exemplo preenchido\n"
        "Receita Médica. Paciente: [PACIENTE]. Data: [DATA]. 1. Amoxicilina + clavulanato "
        "875/125 mg, comprimido, via oral, de 12 em 12 horas, por 7 dias. 2. Dipirona sódica "
        "500 mg, comprimido, via oral, de 6 em 6 horas, se dor ou febre, por até 5 dias. "
        "3. Retorno ambulatorial em 7 dias ou antes se piora clínica. Médico responsável: "
        "[MEDICO] — [CRM].\n\n"
        "## 4. Regras de preenchimento\n"
        "Escrever a posologia por extenso, sem abreviaturas ambíguas (usar unidade internacional "
        "em vez de UI, micrograma em vez de mcg quando manuscrito). Registrar alergias conhecidas "
        "do paciente antes de emitir e conferir interações com as medicações em uso. Não há "
        "receita válida sem data, assinatura e CRM do médico responsável. Sugestões geradas por "
        "sistema de apoio à decisão são rascunho e exigem validação médica antes de qualquer "
        "entrega ao paciente.\n\n"
        "---\n"
    ...
    "procedimento.md": (
        ...
        "---\n\n"
        "## 1. Quando usar\n"
        "Usar este modelo para registrar em prontuário qualquer procedimento invasivo à "
        "beira-leito ou em sala, como acesso venoso central, drenagem torácica, intubação "
        "orotraqueal, punção lombar ou cardioversão elétrica. A descrição é registrada pelo "
        "médico executante imediatamente após o procedimento.\n\n"
        "## 2. Estrutura do documento\n"
        ...
        "    **Médico executante:** {{medico}} — {{crm}}\n\n"
        "## 3. Exemplo preenchido\n"
        "Descrição de Procedimento — Acesso venoso central em veia jugular interna direita. "
        "Paciente: [PACIENTE]. Data: [DATA]. Indicação: necessidade de vasopressor em choque "
        "séptico refratário à reposição volêmica. Técnica: após consentimento, antissepsia com "
        "clorexidina alcoólica e paramentação estéril completa, punção guiada por ultrassom em "
        "veia jugular interna direita, técnica de Seldinger, progressão de cateter de duplo "
        "lúmen até 15 cm, refluxo e infusão testados em ambas as vias, fixação com fio de sutura "
        "e curativo oclusivo. Intercorrências: nenhuma; radiografia de tórax de controle "
        "confirmou ponta do cateter em transição cavoatrial, sem pneumotórax. Médico executante: "
        "[MEDICO] — [CRM].\n\n"
        "## 4. Regras de preenchimento\n"
        "Registrar sempre a indicação clínica que justificou o procedimento, o consentimento "
        "obtido, os cuidados de barreira e o exame de controle quando aplicável. Intercorrências "
        "devem ser descritas mesmo quando resolvidas, incluindo número de tentativas de punção. "
        "Ausência de intercorrência é registrada explicitamente como nenhuma, nunca deixada em "
        "branco. O registro exige assinatura e CRM do médico executante.\n\n"
        "---\n"
        "*Documento sintético para fins acadêmicos — não constitui documento real.*\n"
    ),
}
